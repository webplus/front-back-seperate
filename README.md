...
**所以最终选择了Fis3+Vue，定义了一套适应了多工程多页面的大型工程的开发规范，为其多项目视图量身定制，支持书写es6，es7高级特性,；支持sass，less，coffee等语义糖，自动分析代码依赖，资源定位，支持拓展嵌入Node服务端。**
...
```
  文件引入时，使用`import config from 'env';
           let env = config.env;` from的字符串不要改，否者会找不到env.js
